...
let currentType: string | undefined; // Type de cargaison actuel

// Fonction pour afficher les cargaisons
function afficherCargaisons(cargaisons: Cargaison[], page: number = 1, pageSize: number = 5): void {
...
function fetchCargaisons(type?: string, page: number = 1, pageSize: number = 5): void {
  let url = 'api.php';
  if (type) {
    url += `?type=${type}`;
  }
...
document.getElementById('type-plein')?.addEventListener('change', (event) => {
  const typePlein = (event.target as HTMLSelectElement).value;
  const poidsField = document.getElementById('poids-produit') as HTMLInputElement;
  const nombreField = document.getElementById('nombre-produit') as HTMLInputElement;
  const poidsLabel = document.getElementById('label-poids-produit') as HTMLLabelElement;
  const nombreLabel = document.getElementById('label-nombre-produit') as HTMLLabelElement;
...
function validerChamps(): boolean {
  ...
  const typeCargaison = (document.getElementById('type-cargaison') as HTMLSelectElement).value;
  const typePlein = (document.getElementById('type-plein') as HTMLSelectElement).value;
  const poids_max = (document.getElementById('poids-produit') as HTMLInputElement).value;
  const nombre_produits = (document.getElementById('nombre-produit') as HTMLInputElement).value;
  const lieu_depart = (document.getElementById('depart') as HTMLInputElement).value;
  const lieu_arrivee = (document.getElementById('arrivee') as HTMLInputElement).value;
  ...
  const poidsSuporterError = document.getElementById('poids-cargaison-error') as HTMLSpanElement;
...
document.getElementById('form-add-cargaison')?.addEventListener('submit', (event) => {
  ...
  const typeCargaison:any = (document.getElementById('type-cargaison') as HTMLSelectElement).value;
  const typePlein:any = (document.getElementById('type-plein') as HTMLSelectElement).value;
  const poids_max:any = typePlein === 'poids' ? parseFloat((document.getElementById('poids-produit') as HTMLInputElement).value) : 0;
  const nombre_produits = typePlein === 'nombre' ? parseInt((document.getElementById('nombre-produit') as HTMLInputElement).value) : 0;
  ...
  const lieu_depart:any = (document.getElementById('depart') as HTMLInputElement).value;  
  const lieu_arrivee:any = (document.getElementById('arrivee') as HTMLInputElement).value;  
  const distance_km:any = (document.getElementById('distance') as HTMLInputElement).value;  
  ...
  let newCargaison: Cargaison;
  if (typeCargaison === "Maritime") {
    newCargaison = new CargaisonMaritime(
      numero,
      numero,
      poids_max,
      prix_total,
    ...
    newCargaison = new CargaisonAérienne(
      numero,
      numero,
      poids_max,
      prix_total,
    ...
    newCargaison = new CargaisonRoutière(
      numero,
      numero,
      poids_max,
      prix_total,
